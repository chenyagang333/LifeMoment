...
namespace Chen.Infrastructure.EFCore
{
    public abstract class BaseDbContext : DbContext
    {
        ...
        public async override Task<int> SaveChangesAsync(bool acceptAllChangesOnSuccess, CancellationToken cancellationToken = default)
        {
            ...
            //}
            var softDeleteEntities = ChangeTracker.Entries<ISoftDelete>()
            ...
            // 由于我们这里是通过领域事件去发布集成事件，如果该领域实体修改失败则回滚，同时不会发送领域事件，也就是不发送集成事件
            if (_mediator != null)
            {
                await _mediator.DispatchDomainEventsAsync(this);
            }
            ...
            //就会造成已经软删除的数据仍然能够通过FindAsync查到的问题，因此这里把对应跟踪对象的state改为Detached，
            //就会从缓存中删除了 
            softDeleteEntities.ForEach(e => Entry(e).State = EntityState.Detached);
